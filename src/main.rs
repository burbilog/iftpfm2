...
fn main() {
    ...
    let cli::CliArgs { delete, log_file: log_file_option, config_file: config_file_option,
                       parallel, randomize, grace_seconds, connect_timeout, insecure_skip_verify, upload_verify } =
    ...
    let total_transfers: i32 = pool.install(|| {
        configs_arc
            ...
            .map(|(idx, cf_item)| { // cf_item is a reference to Config
                ...
                transfer_files(cf_item, *delete_arc, thread_id, connect_timeout, insecure_skip_verify, upload_verify)
            })
            .sum()
    });
