...
fn main() {
    ...
    let (delete, log_file_option, config_file_option, parallel, randomize, grace_seconds, connect_timeout) =
    ...
    let total_transfers: i32 = pool.install(|| {
        configs_arc
            ...
            .map(|(idx, cf_item)| { // cf_item is a reference to Config
                ...
                transfer_files(cf_item, *delete_arc, thread_id, connect_timeout)
            })
            .sum()
    });
